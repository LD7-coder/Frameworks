...
function Colocar(matriz, palabra, x, y, dir) {
  palabra = NormalizarPalabra(palabra);
  for (let i = 0; i < palabra.length; i++) {
    if (dir === "H") matriz[x][y + i] = palabra[i];
    else matriz[x + i][y] = palabra[i];
  }
}
...
function ColocarConCruce(matriz, palabra, x, y, dir, indicePalabra) {
  palabra = NormalizarPalabra(palabra);
  for (let i = 0; i < palabra.length; i++) {
    const nx = dir === "H" ? x : x + (i - indicePalabra);
    const ny = dir === "H" ? y + (i - indicePalabra) : y;
...
function ColocarPalabras(matriz, palabras) {
  ...
  Colocar(matriz, primera, startX, startY, "H");
    ...
    for (const c of coincidencias) {
      for (const dir of ["H", "V"]) {
        if (VerSiCabeConCruce(matriz, palabraMayus, c.x, c.y, dir, c.indicePalabra)) {
          ColocarConCruce(matriz, palabra, c.x, c.y, dir, c.indicePalabra);
          colocado = true;
          break;
        }
      }
    ...
    if (!colocado) {
      for (let intento = 0; intento < 100; intento++) {
        ...
        if (VerSiCabeConCruce(matriz, palabraMayus, x, y, dir, 0)) {
          Colocar(matriz, palabra, x, y, dir);
          colocado = true;
          break;
        }
      }
